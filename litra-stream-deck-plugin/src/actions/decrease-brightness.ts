...
/**
 * Action to decrease brightness of Litra lights
 */
@action({ UUID: "com.simon-poirier.litra-stream-deck-plugin.decrease-brightness" })
export class DecreaseBrightness extends SingletonAction<BrightnessSettings> {
    ...
    /**
     * Decrease brightness when the button is pressed
     */
    override async onKeyDown(ev: KeyDownEvent<BrightnessSettings>): Promise<void> {
        ...
        try {
            ...
            for (const device of devices) {
                ...
                const newBrightness = Math.max(1, currentBrightness - step);
            ...
            if (successCount > 0) {
                ...
                console.log(`Decreased brightness for ${successCount} device(s) by ${step}%`);
            } else {
                await ev.action.showAlert();
            }
        } catch (error) {
...
/**
 * Settings for {@link DecreaseBrightness}.
 */
