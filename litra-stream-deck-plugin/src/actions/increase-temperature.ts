...
/**
 * Action to increase temperature of Litra lights
 */
@action({ UUID: "com.simon-poirier.litra-stream-deck-plugin.increase-temperature" })
export class IncreaseTemperature extends SingletonAction<TemperatureSettings> {
    ...
    /**
     * Increase temperature when the button is pressed
     */
    override async onKeyDown(ev: KeyDownEvent<TemperatureSettings>): Promise<void> {
        ...
        try {
            ...
            for (const device of devices) {
                ...
                const newTemp = Math.min(6500, currentTemp + step);
            ...
            if (successCount > 0) {
                ...
                console.log(`Increased temperature for ${successCount} device(s) by ${step}K`);
            } else {
                await ev.action.showAlert();
            }
        } catch (error) {
...
/**
 * Settings for {@link IncreaseTemperature}.
 */
